...
![](images/server/server-login-1-console.png){fig-alt="Screenshot of RStudio's 'Console' pane" fig-align="center" width="85%"}
...
![](images/server/server-login-2-terminal.png){fig-alt="Screenshot of RStudio's 'Terminal' pane" fig-align="center" width="85%"}

### Get Invited!

In order to access the server (and follow the instructions below) we'll need to create an account on the server for you. Please email [someone on our team](https://lter.github.io/scicomp/staff.html) or **scicomp \[at\] nceas.ucsb.edu** so that we can contact NCEAS' IT team and get an invite email sent to you.
...
If you have not received that email, check your Spam folder for emails from Thomas Hetmank (hetmank \[at\] nceas.ucsb.edu) or Nick Outin (outin \[at\] nceas.ucsb.edu). If you have not received the email and it is not in your Spam, reach out to our team and we will work to get an invite sent to you.

![](images/server/server-login-3-invite-email.png){fig-alt="Screenshot of an email with the subject line 'Account on Aurora' and body text including a server host link, username, and password (covered by a red box)" fig-align="center" width="85%"}
...
4. You will then be prompted to change your "expired" password. We consider your temporary password to be expired as soon as you use it because sharing passwords via email is not secure and this "expiration" lets you set the password to something that only you know at the outset of your time in the server. **Note again that the cursor will not advance as you type but it is working!** To update your password, enter your `[temporary password]`, then `[your strong new password]` and finally re-type `[your strong new password]` to confirm it. Note that your new password should not be "your strong new password" {{< fa smile-wink >}}
...
**If you are uploading a _single_ file, it may be most convenient to use RStudio's data upload capabilities.** In the "Files" pane (default is top right but it may differ on your computer), you can click the button that is a white rectangle with a **<span style="color:gold">yellow</span>** circle with an upward facing arrow inside of it. In the below image the relevant button is in a **<span style="color:red">red</span>** square.

![](images/server/server-upload-1.png){fig-alt="Screenshot of RStudio's 'Files' tab where there is a red box drawn around the 'upload data' button (a small white square with a yellow, upward-facing arrow)" fig-align="center" width="75%"}

Clicking that button allows you to choose (1) where you want the uploaded file to go and (2) which file you want to upload.

![](images/server/server-upload-2.png){fig-alt="Screenshot of RStudio's file upload dialog window. Looks much like any other file attach/upload interface (e.g., attaching a file to an email)" fig-align="center" width="75%"}

You can also download data via the RStudio interface. To download a file, check the box next to it in the "Files" pane.

![](images/server/server-download-1.png){fig-alt="Screenshot of RStudio's 'Files' pane where the box next to one file is checked" fig-align="center" width="75%"}
...
![](images/server/server-download-2.png){fig-alt="Screenshot of the RStudio menu visible after clicking the 'Gear' icon in the 'Files' pane, where the 'Export...' option is surrounded by a red rectangle" fig-align="center" width="75%"}
...
![](images/server/server-download-3.png){fig-alt="Screenshot of RStudio's file export menu where the file to be downloaded is being renamed" fig-align="center" width="65%"}
...
![](images/server/server-cyberduck-1.png){fig-alt="Screenshot of the top menu bar of Cyberduck"}
...
![](images/server/server-cyberduck-2.png){fig-alt="Screenshot of Cyberduck's 'FTP' setup menu. Contains fields for the server name, username, and password" fig-align="center" width="85%"}
...
![](images/server/server-cyberduck-3.png){fig-alt="Screenshot of Cyberduck's FTP setup page where a dropdown menu of different types of file transfer protocols is open and 'SFTP (SSH File Transfer Protocol)' is selected" fig-align="center" width="85%"}
...
1. Navigate to the folder <u>on your computer</u> containing the file(s) you want to upload
...
2. Now you can upload that file to the server!
...
1. Navigate to the folder containing the folder on your computer you want to upload (using `cd`)

```
cd [folder]
```

2. Use `scp` with the recursive option (`-r`) to upload all of the files in that folder
    - Note that this uploads all of the files in the folder _not_ the folder itself so you may want to create a new folder on the server to upload your local folder's contents into.
...
You can also _download_ files from the server to your local machine by simply switching the order of the machines from how you upload many files to the server.
...
The `get` command can be used to _download_ a file.
...
5. When you are done downloading, close the connection to the server
...
1. You can also upload a local file to the server by using the `put` sFTP command

```
put [file_name_local]
```

2. When you are done uploading, close the connection to the server
