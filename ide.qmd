...
Once you have clicked that button and opened its dropdown menu, click **New Project...**
...
Let's try out a simple Git workflow by first creating a new file. This is **Step 1** of the process. To begin, go to your IDE of choice and open it.

::::{.panel-tabset}
### In RStudio

In RStudio, you can make a file by going to **File** > **New File** > **R Script**. Feel free to type anything you want into this script as an example. Name this script after yourself. In the screenshot below, I have named my script `angel-script.R`. 

Once you are done, navigate to the **Git** tab on the upper right corner. You should see your new script show up there along with a `.gitignore` and `git-practice.Rproj` file. Do not worry about the `.gitignore` file for now, it was created by RStudio to make sure that some unwanted files are not tracked by Git. The `git-practice.Rproj` file will save your settings and open tabs when you close the project, and will restore these settings the next time you open it.
...
Notice that there are color-coded icons next to the files in the "Git" tab. These icons are shorthand for the Git status for any file whose status has changed since the last commit that you made.
...
<p align="center">
<img src="images/commit/rstudio-02_diff.png" alt="Screenshot of the 'diff' button--circled in red--in the 'Git' pane of RStudio" width="60%"/>
</p>
...
**Step 2**: Add the file to the next commit (i.e., "stage" the edits to that file) by checking the box in front of the file name. Note that the two `?` icons will change to a single `A` on the left to show you that this file is now staged to be part of the next commit.
...
<p align="center">
<img src="images/commit/rstudio-03_commit-message.png" alt="Screenshot of the commit menu of RStudio where one file has been staged and an informative commit message has been entered" width="80%"/>
</p>

If all of the above steps went well, you should see something like this:

<p align="center">
<img src="images/commit/rstudio-04_commit-message-success.png" alt="Screenshot of the success message after a commit has been made" width="80%"/>
</p>
...
<p align="center">
<img src="images/commit/rstudio-05_pull-push-button.png" alt="Screenshot of the blue 'pull' button and green 'push' button in RStudio" width="40%"/>
</p>

In my case, it turns out that a new script, `lyon-script.R`, was added to the GitHub repository by a collaborator while I was making my own script. The pull will succeed because there is no conflict but RStudio will let us know that it retrieved a copy of the file when we pulled.

<p align="center">
<img src="images/commit/rstudio-06_pulled-message.png" alt="Screenshot of the message returned when you pull files you didn't have locally from GitHub" width="70%"/>
</p>

Since I have just pulled, `lyon-script.R` now shows up in my RStudio files. A new message has also popped up for me: "Your branch is ahead of 'origin/main' by two commits". This means that I have two additional commits on my local machine that I never shared back to the remote repository on GitHub. If I look at the content of my repository on GitHub, I will see just the `README.md` and `lyon-script.R`. My changes are _not_ in GitHub yet. You might be seeing a similar message as well.

<p align="center">
<img src="images/commit/rstudio-06b_pulled-file.png" alt="Screenshot of RStudio with the 'git' pane underlined in red where it says 'your branch is ahead of origin/main by 2 commits'" width="100%"/>
</p>

#### Sending Changes back to GitHub

So how do we send our changes back to GitHub? Locate the **Push** button on the "Git" tab and click on it. Now your script should show up in the GitHub repository! Once you click that button you should get a success screen like the one pictured below.

<p align="center">
<img src="images/commit/rstudio-07_push.png" alt="Screenshot of the message received when a push is performed successfully from RStudio" width="70%"/>
</p>
...
<p align="center">
<img src="images/commit/github-02b_updated-repo.png" alt="Screenshot of a GitHub repository with the commits made in earlier screenshots shown in its history" width="100%"/>
</p>
...
In Positron, you can make a file by going to **File** > **New File...** > and choosing the type of file that you want to create. Feel free to type anything you want into this script as an example. Name this script after yourself. In the screenshot below, I have named my script `lyon-script.R`.

Once you are done making the file, navigate to the "Source Control" ({{< fa code-branch >}}) piece of the left sidebar of Positron. You should see your new script show up there. Your other files all still exist in your working directory but this facet of Positron will only identify files that have changed since your last commit.

<p align="center">
<img src="images/commit/positron-01_new-file.png" alt="Screen capture of Positron source control page with one new file" width="100%"/>
</p>

Note that there are abbreviations next to the files in the "Source Control" facet of Positron. These indicate the Git status for any file whose status has changed since the last commit that you made. You should see "U" next to your new file because it is currently "untracked" by Git.

#### Adding Our Script to the Next Commit

We would like to save a snapshot of this version of our script. Since we’ve just done **Step 1**, here are the rest of the steps we will need to do to get our script to show up on our GitHub repository:

**Step 2**: Add the file to the next commit (i.e., “stage” the edits to that file) by clicking the plus sign ({{< fa plus >}}) next to the file's name in the source control menu. Note that the `U` will turn into an `A` to show you that this file is now staged to be part of the next commit.

**Step 3**: In the left sidebar, above where you staged the file, type a short but descriptive commit message detailing what you have done so far. Then click on the "Commit" button to save this version of the script in the Git database.

<p align="center">
<img src="images/commit/positron-02_commit-message.png" alt="Screen capture of Positron source control pane with a staged file" width="100%"/>
</p>

If that commit went well, the "Commit" button should now be a "Sync Changes" button.

<p align="center">
<img src="images/commit/positron-03_ready-to-sync.png" alt="Screen capture of Positron source control pane with the 'sync changes' button active" width="100%"/>
</p>
...
<p align="center">
<img src="images/commit/positron-04_sync-alert.png" alt="Screen capture of Positron warning about what 'sync changes' will do" width="40%"/>
</p>

You will know that the sync worked because the file that you just committed will be absent from the "changes" list. In our case, because that was the only file with any changes, it means there is nothing in that list and the "Commit" button is grayed out and inactive.

<p align="center">
<img src="images/commit/positron-05_synced.png" alt="Screen capture of Positron source control pane when there are no new changes to be committed or synced" width="70%"/>
</p>
